    ...
    try:
        while True:
            ...
            if msg.error():
                ...
                existing_email = cassandra_connector.session.execute(
                    query, (email,)).one()
...
def kafka_consumer_cassandra_main():
    cassandra_connector = CassandraConnector(['192.168.135.159'])

    cassandra_connector.create_keyspace()
    cassandra_connector.create_table()

    kafka_config = {
        'bootstrap.servers': '192.168.135.159:9092',
    ...
    fetch_and_insert_messages(
        kafka_config, cassandra_connector, topic, run_duration_secs)
