        ...
        try:
            while True:
                ...
                if msg.error():
                    ...
                    existing_document = self.mongodb_connector.db[self.mongodb_connector.collection_name].find_one(
                        {"email": email, "otp": otp})
                    if existing_document:
                        logger.warning(
                            f"Document with Email={email}, OTP={otp} already exists in the collection.")
                    else:
                        self.mongodb_connector.insert_data(email, otp)
                        logger.info(
                            f'Received and inserted: Email={email}, OTP={otp}')
...
mongodb_uri = 'mongodb://192.168.135.159:27017/'
...
def kafka_consumer_mongodb_main():
    ...
    kafka_consumer = KafkaConsumerWrapperMongoDB(
        kafka_config, topics, mongodb_connector)
